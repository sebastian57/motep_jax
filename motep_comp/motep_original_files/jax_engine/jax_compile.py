...
def extract_static_params(level, mtp_file, MAX_ATOMS, MAX_NEIGHBORS):
    mtp_data = read_mtp(f'training_data/{mtp_file}.mtp')
    ...
    return execution_order, scalar_contractions
    
@partial(jax.jit, static_argnames=('level','mtp_path','MAX_ATOMS','MAX_NEIGHBORS'))
def complete_calc_energy_forces_stress(itypes,
    all_js,
    all_rijs,
    all_jtypes,
    cell_rank,
    ...
    execution_order, scalar_contractions = extract_static_params(level,mtp_path,MAX_ATOMS,MAX_NEIGHBORS)

    mtp_data = read_mtp(f'{mtp_path}.mtp')

    jax.debug.print('mtp_data: {}', mtp_data)

    local_energies, forces, stress_voigt = calc_energy_forces_stress(
    itypes,
    all_js,
    all_rijs,
    all_jtypes,
