...
def calc_energy_forces_stress_padded_simple(
    ...
    energies, forces, stress = calc_energy_forces_stress(
        itypes,
        all_js,      # Trust C++ neighbor indices
        all_rijs,    # Trust C++ distance vectors  
        all_jtypes,  # Trust C++ neighbor types
        cell_rank,
        volume,
        species,
        scaling,
...
def calc_energy_forces_stress_padded(
    ...
    itypes_masked = jnp.where(atom_mask, itypes, 0)
    
   
    safe_neighbor_indices = jnp.arange(MAX_ATOMS)[:, None] 
    neighbor_mask = jnp.arange(MAX_NEIGHBORS)[None, :] < nneigh_actual
    all_js_safe = jnp.where(
        atom_mask[:, None] & neighbor_mask,
        all_js,
        safe_neighbor_indices 
    )
    
    large_distance = max_dist + 1.0 
    safe_distances = jnp.full((MAX_ATOMS, MAX_NEIGHBORS, 3), large_distance)
    all_rijs_safe = jnp.where(
        (atom_mask[:, None] & neighbor_mask)[:, :, None],
        all_rijs,
        safe_distances
    )
    
    all_jtypes_safe = jnp.where(
        atom_mask[:, None] & neighbor_mask,
        all_jtypes,
        0  
    )
    ...
    energies_masked = jnp.where(atom_mask, energies, 0.0)
    forces_masked = jnp.where(atom_mask[:, None], forces, 0.0)
    
    energy = energies_masked.sum()
    
    return energy, forces_masked, stress
...
def calc_energy_forces_stress(
    ...
    local_energies, forces_per_neighbor = _jax_calc_local_energy_and_derivs(
        all_rijs,
        itypes,
        all_jtypes,
        species_coeffs,
...
def calc_energy_forces_stress_old(
    itypes,
    all_js,
    all_rijs,
    all_jtypes,
...
def _jax_calc_local_energy_and_derivs(
    ...
    derivs = jax.jacobian(_jax_calc_local_energy, argnums=0)(
        r_ijs,
        itype,
        jtypes,
        species_coeffs,
...
@partial(jax.vmap, in_axes=(0,) * 3 + (None,) * 11, out_axes=0)
def _jax_calc_local_energy_and_derivs_(
    r_ijs,
    itype,
    jtypes,
    species_coeffs,
    ...
    local_energies = jnp.full(itypes_shape, total_energy)
