/* ----------------------------------------------------------------------
   ...
   Direct JAX Implementation - Production Version with GPU Support
------------------------------------------------------------------------- */
...
#include <algorithm>
#include <sstream>      // For std::istringstream - ADDED THIS
...
void PairJaxMTPDirect::init_python_direct()
{
  ...
  if (comm->me == 0) {
    utils::logmesg(lmp, "=== ULTRA SIMPLE DEBUG ===\\n");
  }
  ...
  // Test 1: Environment
  PyObject *env_test = PyRun_String("__import__('os').environ.get('JAX_PLATFORMS', 'NOT_SET')", Py_eval_input, PyDict_New(), PyDict_New());
  if (env_test && comm->me == 0) {
    const char* env_result = PyUnicode_AsUTF8(env_test);
    utils::logmesg(lmp, "🔍 JAX_PLATFORMS in embedded Python: {}\\n", env_result);
    Py_DECREF(env_test);
  }
  
  // Test 2: JAX import
  bool jax_imported = false;
  ...
  if (jax_module) {
    ...
    // Test 3: JAX version
    PyObject *version_attr = PyObject_GetAttrString(jax_module, "__version__");
    if (version_attr && comm->me == 0) {
      const char* version = PyUnicode_AsUTF8(version_attr);
      utils::logmesg(lmp, "   JAX version: {}\\n", version);
    ...
    if (devices_func) {
      ...
      if (devices_result && comm->me == 0) {
        ...
        // Check if GPU in devices string
    ...
    // Test 5: Backend platform
    PyObject *xla_bridge = PyImport_ImportModule("jax.lib.xla_bridge");
    if (xla_bridge) {
      PyObject *get_backend_func = PyObject_GetAttrString(xla_bridge, "get_backend");
      if (get_backend_func) {
        PyObject *backend = PyObject_CallObject(get_backend_func, nullptr);
        if (backend) {
          PyObject *platform_attr = PyObject_GetAttrString(backend, "platform");
          if (platform_attr && comm->me == 0) {
            const char* platform = PyUnicode_AsUTF8(platform_attr);
            utils::logmesg(lmp, "   XLA backend platform: {}\\n", platform);
            Py_DECREF(platform_attr);
          }
          Py_DECREF(backend);
        }
        Py_DECREF(get_backend_func);
      }
      Py_DECREF(xla_bridge);
    }
  ...
  if (comm->me == 0) {
    utils::logmesg(lmp, "=== END ULTRA SIMPLE DEBUG ===\\n");
  }

  // Continue with JAX export module loading
  jax_export_module = PyImport_ImportModule("jax.export");
  if (jax_export_module == nullptr) {
    PyErr_Print();
    error->one(FLERR, "Failed to import jax.export module after restart");
  }
  ...
  if (comm->me == 0) {
    utils::logmesg(lmp, "🚀 JAX potential initialized\\n");
