...
@jax.jit
def extract_unique_map(sym_tensor, unique_indices):
    unique_elements = sym_tensor[tuple(unique_indices.T)]
...
counter_map = 0
for nu in nus:
    print(f'starting map timing: {nu}')
    ...
    unique_indices = load_data_pickle(f'tensor_maps/{nu}_map.pkl')
    end_time_ind = time.time()
    elapsed_time_ind = end_time_ind - start_time_ind
    print(elapsed_time_ind)

    start_time_el = time.time()
    unique_elements = extract_unique_map(tensor, unique_indices[nu])
    end_time_el = time.time()
    elapsed_time_el = end_time_el - start_time_el
    print(elapsed_time_el)

    start_time_rec = time.time()
    reconstructed_tensor = reconstruct_symmetric_tensor_scan(unique_elements, unique_indices[nu], nu)
    ...
    map_indices_times.append(elapsed_time_total)

    counter_map += 1

nus = np.arange(2,8)
map_indices_times_cached = []
counter_map_cache = 0
for nu in nus:
    print(f'starting cached map timing: {nu}')
    tensors = _jax_make_tensor(r,nu)
    tensor = tensors[0]
    ...
    map_indices_times_cached.append(elapsed_time_total)
...
print(map_indices_times)
print(map_indices_times_cached)



fig, ax = plt.subplots()

ax.plot(nus[0:counter_insitu], insitu_indices_times, 'x-', c='r', label='insitu')
ax.plot(nus[0:counter_map], map_indices_times, 'x-', c='b', label='map')

ax.plot(nus[0:counter_insitu_cache], insitu_indices_times_cached, 'x-', c='r', label='insitu cached')
ax.plot(nus[0:counter_map_cache], map_indices_times_cached, 'x-', c='b', label='map cached')
