...
class HighlyOptimizedMTPLammpsInterface:
    def __init__(self, mtp_file, use_mpi=True, cache_dir="./jax_cache", 
                 chunk_size=50000, overlap_comm=True, memory_fraction=0.8):
    ...
    def _setup_jax_devices(self):
        ...
        device_per_rank = self.n_devices // max(1, self.size)
        self.assigned_devices = self.devices[self.rank * device_per_rank:(self.rank + 1) * device_per_rank]
    ...
    def _initialize_mtp_cached(self):
        ...
        cache_file = os.path.join(self.cache_dir, f"mtp_cache_rank{self.rank}.pkl")
        
        if os.path.exists(cache_file) and self.rank == 0:
        ...
        if self.comm:
            ...
            data_to_broadcast = self.comm.bcast(data_to_broadcast, root=0)
            
            if self.rank != 0:
                self.mtp_data = data_to_broadcast['mtp_data']
                self.params = data_to_broadcast['params']
                self.mtp_instance = data_to_broadcast['mtp_instance']
    ...
    def _parallel_jax_compilation(self):
        ...
        test_sizes = [100, 1000, 10000, 50000] if self.rank == 0 else [self.chunk_size]
    ...
    def _compile_with_pmap(self, test_sizes):
        ...
        for size in test_sizes:
            size_per_device = size // n_devices
    ...
    def _setup_async_buffers(self):
        if not self.comm:
            return
            
        self.async_send_buffer = np.empty((self.chunk_size, 3), dtype=np.float32)
        self.async_recv_buffer = np.empty((self.chunk_size, 3), dtype=np.float32)
        self.async_requests = []

    def force_callback(self, caller, ntimestep, nlocal, tag, x, f):
        
        if ntimestep % 100 == 0 and self.rank == 0:
            print(f"Step {ntimestep}: Computing forces for {nlocal} atoms across {self.size} ranks")
    ...
    def _fast_data_transfer_in(self, x, nlocal):
        if hasattr(x, '__array_interface__'):
            self.current_positions = np.frombuffer(
                x.__array_interface__['data'][0], 
                dtype=np.float64, 
                count=nlocal*3
            ).reshape((nlocal, 3)).astype(np.float32)
        else:
            self.current_positions = self.position_buffer[:nlocal]
            self.current_positions[:] = x[:nlocal].astype(np.float32)

    def _fast_data_transfer_out(self, f, forces, nlocal):
        if hasattr(f, '__array_interface__') and forces.dtype == np.float64:
            f_view = np.frombuffer(
                f.__array_interface__['data'][0], 
                dtype=np.float64, 
                count=nlocal*3
            ).reshape((nlocal, 3))
            f_view[:] = forces[:nlocal]
        else:
            f[:nlocal] = forces[:nlocal].astype(np.float64)
    ...
    def _process_chunked(self, nlocal):
        ...
        for i in range(n_chunks):
            ...
            if self.overlap_comm and self.comm and i < n_chunks - 1:
                self._start_async_communication(chunk_forces, i)
        
        if self.overlap_comm and self.async_requests:
            MPI.Request.Waitall(self.async_requests)
    ...
    def _vectorized_neighbor_processing(self, nl, atoms):
        ...
        estimated_max_neighbors = min(100, n_atoms)  # Reasonable estimate
    ...
    def _start_async_communication(self, data, chunk_id):
        if not self.comm or self.size == 1:
            return
        
        next_rank = (self.rank + 1) % self.size
        req = self.comm.Isend(data, dest=next_rank, tag=chunk_id)
    ...
    def _print_performance_snapshot(self, timestep, total_time, nlocal):
        ...
        print(f"\n=== PERFORMANCE SNAPSHOT (Step {timestep}) ===")
        print(f"Atoms: {nlocal}, Ranks: {self.size}, Devices: {len(self.assigned_devices)}")
        ...
        print(f"Total system: {atoms_per_second * self.size:.0f} atoms/second")
