...
class MTPLammpsInterface:
    def __init__(self, lmp, level=2, species=None, min_dist=0.5, max_dist=5.0, scaling=1.0):
    ...
    def _initialize_mtp(self):
        ...
        untrained_mtp = f'motep_jax/untrained_mtps/{level_formatted}.mtp'
        rng = np.random.default_rng(10)
        
        self.mtp_data = read_mtp(untrained_mtp)
        ...
        self.mtp_data.min_dist = self.min_dist
        self.mtp_data.max_dist = self.max_dist
        self.mtp_data.scaling = self.scaling
        self.mtp_data.initialize(rng)
        
        
        with open('mtp_params.pkl', 'rb') as f:
            loaded_params = pickle.load(f)
        
        self.params = {
            'species': loaded_params['species'],
            'radial': loaded_params['radial'],
            'basis': loaded_params['basis']
        }
    ...
    def setup_simulation(self, input_file=None):
        """Setup LAMMPS simulation with external force callback"""
        if input_file:
            self.lmp.file(input_file)
        else:
            self.lmp.command("units metal")
            self.lmp.command("atom_style atomic")
            self.lmp.command(f"neighbor {self.max_dist/2:.2f} bin")
            self.lmp.command("neigh_modify delay 0 every 1 check yes")
            
        # Create external force fix
        self.lmp.command("fix ext_force all external pf/callback 1 1")
        print("LAMMPS simulation setup complete with external force callback")
    ...
    def load_params_from_file(self, params_file):
        """Load MTP parameters from a file"""
        with open(params_file, 'rb') as f:
            params = pickle.load(f)
        
        self.loaded_params = params
        print(f"Loaded MTP parameters from {params_file}")
    ...
    def print_performance_stats(self):
        ...
        print(f"Total time in MTP potential: {self.total_time:.2f}s")
        print(f"Number of calls: {self.call_count}")
        print(f"Average time per call: {avg_time:.4f}s")
        
        natoms = self.lmp.get_natoms()
        print(f"Approximate time per atom per call: {avg_time/natoms*1e6:.2f} microseconds")
...
def main():
    ...
    species = [0,1] 
    mtp_interface = MTPLammpsInterface(
        lmp, 
        level=18,
        species=species,
        min_dist=0.5,
    ...
    mtp_interface.setup_simulation(input_file='in.nvt_heat_dump')
    lmp.set_fix_external_callback("ext_force", mtp_interface.force_callback)
    
    lmp.command('run 500')
    
    mtp_interface.print_performance_stats()
